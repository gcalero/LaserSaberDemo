...
LaserSaberApp::LaserSaberApp(JNIEnv* env, jobject asset_mgr_obj, jlong gvr_context_ptr)
...
void LaserSaberApp::OnDrawFrame() {
    ...
    glm::mat4 glmHeadView = glm::make_mat4(&(Utils::MatrixToGLArray(head_view)[0])); //glm::inverse(glm::make_mat4(&(Utils::MatrixToGLArray(head_view)[0])));
    glm::mat4 glmLeftEyeFromHeadMatrix = glm::make_mat4(&Utils::MatrixToGLArray(gvr_api_->GetEyeFromHeadMatrix(GVR_LEFT_EYE))[0]);
    glm::mat4 glmLeftEyeView = glmLeftEyeFromHeadMatrix * glmHeadView;

    glm::mat4 glmRightEyeFromHeadMatrix = glm::make_mat4(&Utils::MatrixToGLArray(gvr_api_->GetEyeFromHeadMatrix(GVR_RIGHT_EYE))[0]);
    glm::mat4 glmRightEyeView = glmRightEyeFromHeadMatrix * glmHeadView;
    ...
    glm::mat4 proj_matrix =   glm::make_mat4(&Utils::MatrixToGLArray(Utils::PerspectiveMatrixFromView(
            scratch_viewport_.GetSourceFov(),
            NEAR_CLIP,
            FAR_CLIP))[0]);

    skybox_.draw(GVR_LEFT_EYE, proj_matrix, glmLeftEyeView, scratch_viewport_, framebuf_size_);

    viewport_list_.GetBufferViewport(1, &scratch_viewport_);

    proj_matrix =   glm::make_mat4(&Utils::MatrixToGLArray(Utils::PerspectiveMatrixFromView(
            scratch_viewport_.GetSourceFov(),
            NEAR_CLIP,
            FAR_CLIP))[0]);

    skybox_.draw(GVR_RIGHT_EYE, proj_matrix, glmRightEyeView, scratch_viewport_, framebuf_size_);
...
void LaserSaberApp::PrepareFramebuffer() {
...
/*void LaserSaberApp::CheckColorSwitch() {
...
/*void LaserSaberApp::CheckChangeStrokeWidth() {
...
/*void LaserSaberApp::AddVertex(const std::array<float, 3>& coords, float u, float v) {
...
/*void LaserSaberApp::AddPaintSegment(const std::array<float, 3>& start_point,
...
/*void LaserSaberApp::StartPainting(const std::array<float, 3> paint_start_pos) {
    if (painting_) return;
    painting_ = true;
    paint_anchor_ = paint_start_pos;
}*/

/*void LaserSaberApp::StopPainting(bool commit_cur_segment) {
    if (!painting_) return;
...
void LaserSaberApp::ClearDrawing() {
...
void LaserSaberApp::OnSkyboxImageReady(int width, int height, jint pos, u_char * data) {
    skybox_.addImage(width, height, pos, data);
}
